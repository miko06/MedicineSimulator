...
interface DiagnosisRecord {
  id: string; nameEn: string; nameRu: string; specialty: { id: string; slug: string; nameEn: string };
  specialtyId: string; descriptionEn: string; descriptionRu: string;
  treatmentsEn: string[]; treatmentsRu: string[];
}

interface SpecialtyShort { id: string; slug: string; nameEn: string; nameRu: string; }
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-mono text-accent">Diagnoses</h1>
      ...
      {showForm && (
        <div className="glass rounded-xl p-6 mb-6">
          <h3 className="text-sm font-mono text-accent mb-4">{editing ? "Edit" : "New"} Diagnosis</h3>
          <div className="grid grid-cols-2 gap-4">
            ...
            <input className="bg-surface border border-border rounded px-3 py-2 text-sm text-text outline-none focus:border-accent" placeholder="Treatments EN (comma-sep)" value={form.treatmentsEn} onChange={e => setForm({...form, treatmentsEn: e.target.value})} />
            <textarea className="bg-surface border border-border rounded px-3 py-2 text-sm text-text outline-none focus:border-accent" placeholder="Description EN" value={form.descriptionEn} onChange={e => setForm({...form, descriptionEn: e.target.value})} rows={2} />
            <textarea className="bg-surface border border-border rounded px-3 py-2 text-sm text-text outline-none focus:border-accent" placeholder="Description RU" value={form.descriptionRu} onChange={e => setForm({...form, descriptionRu: e.target.value})} rows={2} />
            <input className="bg-surface border border-border rounded px-3 py-2 text-sm text-text outline-none focus:border-accent col-span-2" placeholder="Treatments RU (comma-sep)" value={form.treatmentsRu} onChange={e => setForm({...form, treatmentsRu: e.target.value})} />
          </div>
          ...
          <tbody>
            {diagnoses.map((d) => (
              <tr key={d.id} className="border-b border-border/50 hover:bg-surface/50">
                ...
                <td className="px-4 py-3 text-muted text-xs">{d.specialty?.nameEn}</td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => { setEditing(d); setForm({ nameEn: d.nameEn, nameRu: d.nameRu, specialtyId: d.specialtyId, descriptionEn: d.descriptionEn, descriptionRu: d.descriptionRu, treatmentsEn: (d.treatmentsEn||[]).join(", "), treatmentsRu: (d.treatmentsRu||[]).join(", ") }); setShowForm(true); }} className="text-xs text-muted hover:text-text mr-3">Edit</button>
                  <button onClick={() => handleDelete(d.id)} className="text-xs text-red-400 hover:text-red-300">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
